...
function mb_eaw_names($string, $encoding = null)
{
    ...
    if ($eaw_table === null) {
        ...
        $value_iterator = new ArrayIterator(str_split($assoc_values));
        foreach (explode(',', $assoc_keys) as $value) {
            $eaw_table['assoc'][base_convert($value, 36, 10)] = $value_iterator->current();
            $value_iterator->next();
        }
